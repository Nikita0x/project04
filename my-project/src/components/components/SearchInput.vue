<template>
	<nav class="p-5">
		...
		<div class="relative shadow-xl">
			...
			/>
			<button
				@click="fetchWeatherCoordinates"
				class="text-white absolute right-2.5 bottom-2.5 bg-[#050708] hover:bg-[#050708]/80 focus:ring-4 focus:outline-none focus:ring-[#050708]/50 font-medium rounded-lg text-sm px-4 py-2 dark:hover:bg-[#050708]/40 dark:focus:ring-gray-600"
			>
				Search
			</button>
		</div>
...
<script setup>
	const API_KEY = "abeed5ac2959f8c51021124fa14da439";
	const TIMEZONEDB_KEY = "ZSI4X38NTBVZ";
	const OPENCAGEDATA_KEY = "d04e7edb86a8478380dba9c35ffbf0c8";
	const PIXABAY_API = "43375381-a0b8f14c3fa9a8eb8c546762b";
	...
	async function fetchWeatherCoordinates() {
		...
		try {
			...
			console.error(
				"could not retireve data from openweatherapi + ",
				error,
			);
		}
		...
		async function fetchTimezonedb() {
			...
			try {
				const response = await fetch(
					`https://api.timezonedb.com/v2.1/get-time-zone?key=${TIMEZONEDB_KEY}&format=json&by=position&lat=${
						lat.value
					}&lng=${lon.value}&page=${page++}`,
				);
				const data = await response.json();
